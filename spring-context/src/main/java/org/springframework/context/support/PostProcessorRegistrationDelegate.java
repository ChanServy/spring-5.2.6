...
final class PostProcessorRegistrationDelegate {
	...
	//执行参数中，以及Spring容器中所有的工厂后处理器BeanFactoryPostProcessor的方法
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			for (String ppName : postProcessorNames) {
				//1.1):找到这些实现了接口BeanDefinitionRegistryPostProcessor的实现类中，同时又实现了接口PriorityOrdered的实现类
				if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
					//1.1.1）：添加到当前处理的BeanDefinitionRegistryPostProcessor集合中
					currentRegistryProcessors.add(beanFactory.getBean(ppName, BeanDefinitionRegistryPostProcessor.class));
					//1.1.2）：记录类型为BeanDefinitionRegistryPostProcessor的bean的名称（避免后续相同的bean重复被执行了）
					processedBeans.add(ppName);
				}
			}
			//2):根据PriorityOrdered或Ordered接口进行排序
			sortPostProcessors(currentRegistryProcessors, beanFactory);
			//3):添加到registryProcessors集合中（方便后续执行方法postProcessBeanFactory）
			registryProcessors.addAll(currentRegistryProcessors);
			//4):执行currentRegistryProcessors中，第一阶段的方法：postProcessBeanDefinitionRegistry
			...
			//同时，这些实现类不能包含前面已经处理了的、也就是实现了PriorityOrdered接口和Ordered接口的那些实现类
			boolean reiterate = true;
			while (reiterate) {
				reiterate = false;
				postProcessorNames = beanFactory.getBeanNamesForType(BeanDefinitionRegistryPostProcessor.class,
						true, false);
				//直到postProcessorNames为空，reiterate保持为false，才推出while循环
				for (String ppName : postProcessorNames) {
		...
		//2.2:存放实现了Ordered接口的BeanFactoryPostProcessor接口实现类名称
		...
		for (String ppName : postProcessorNames) {
			if (processedBeans.contains(ppName)) {
				...
				//直接空实现，因为processedBeans中，记录了前面处理BeanDefinitionRegistryPostProcessor的bean的名称
				//表示已经处理过了，这里就不再重复处理
			}
		...
		//2.6:根据刚才记录的实现Ordered接口的bean的名称，从beanFactory中获取对应的bean
		...
		//2.7:根据刚才记录的无序的bean的名称，从beanFactory中获取对应的bean
		...
		// 2.8：清除元数据相关的缓存，后处理器可能已经修改了原始的一些元数据
	...
	public static void registerBeanPostProcessors(
		...
		//得到最终容器中的bean后处理器的数量：
		// 容器中已注册的bean后处理器数量 + 即将要注册的后处理器BeanPostProcessorChecker + 容器中还没有注册的bean后处理器数量
		...
		List<String> orderedPostProcessorNames = new ArrayList<>();
		//存放无序的的bean后处理器BeanPostProcessor
		List<String> nonOrderedPostProcessorNames = new ArrayList<>();
		...
		//5.排序并将实现了接口PriorityOrdered的bean后处理器，注册到容器beanFactory中
