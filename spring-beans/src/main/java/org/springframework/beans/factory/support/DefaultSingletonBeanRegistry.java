...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		...
		synchronized (this.singletonObjects) {
			...
			if (singletonObject == null) {
				//如果发现当前单例bean正在销毁，则不允许创建单例bean（默认为false)
	...
	/**
	 * 返回结合singletonsCurrentlyInCreation中，是否存在beaName
	 * 也就是判断当前beaName对应的bean，是否正在实例化
	...
	/**
	 * 注册依赖的bean，方便bean在销毁前，提前销毁bean依赖的那些bean
	...
	private boolean isDependent(String beanName, String dependentBeanName, @Nullable Set<String> alreadySeen) {
		...
		//当前bean的依赖中，已经包含该bean的名称，直接返回true
		...
		for (String transitiveDependency : dependentBeans) {
			...
			//判断dependentBeanName是否存在于beanName的依赖链上
