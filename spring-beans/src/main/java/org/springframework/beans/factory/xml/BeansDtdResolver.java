...
public class BeansDtdResolver implements EntityResolver {
	...
	public InputSource resolveEntity(@Nullable String publicId, @Nullable String systemId) throws IOException {
		...
		//1.判断systemId不为空，并且是以.dtd为后缀的
		if (systemId != null && systemId.endsWith(DTD_EXTENSION)) {
			int lastPathSeparator = systemId.lastIndexOf('/');
			//2.定位spring-beans在字符串systemId中的位置
			int dtdNameStart = systemId.indexOf(DTD_NAME, lastPathSeparator);
			if (dtdNameStart != -1) {
				//3.拼接dtd文件的名称：spring-beans.dtd
				String dtdFile = DTD_NAME + DTD_EXTENSION;
				if (logger.isTraceEnabled()) {
					logger.trace("Trying to locate [" + dtdFile + "] in Spring jar on classpath");
				}
				try {
					//4.从classpath中，将spring-beans.dtd声明文件，封装为资源ClassPathResource
					Resource resource = new ClassPathResource(dtdFile, getClass());
					//5.创建InputSource，将dtd对应的输入流、publicId和systemId都封装到InputSource中
